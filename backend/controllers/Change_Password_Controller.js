...
export const Admin_FirstPassword = async (req, res) => {
    const {id, role, newPassword} = req.body;
    ...
    try {
        ...
        if(["head librarian", "it librarian", "assistant librarian"].includes(role.toLowerCase())){
           ...
           .findOneAndUpdate(
            ...
            if(!updateUser){
                return res.status(404).json({ message: "Librarian not found", isSuccess: false });
            }
...
export const Admin_ChangePassword = async (req, res) => {
       try {
         const {id, currentPassword, newPassword} = req.body;
         console.log('Check Id:', id);
         console.log('Check Current:', currentPassword);
         ...
         const user = await Librarian_Model.findById(id);
         if(!user){
            res.status(400).json({message: "The user can't found."});
            return  
         }
