...
const HistoryTable = ({Returned}) => {
    ...
    {Returned.length > 0 ? (
        Returned.map((borrow) => (
            <div
                key={borrow._id}
                 className={`w-full rounded-lg p-2 flex bg-stone-50 justify-between items-start  border border-stone-300 gap-2`}
            >
        ...
        <div className="w-full flex flex-col justify-center items-center py-6 bg-stone-50 border border-stone-200 rounded-lg">
            <h1 className="text-stone-700 font-semibold text-[10px]">No History/Record found.</h1>
            <h1 className="text-stone-500 text-[10px]">No one return the book yet.</h1>
        </div>
    )}
</div>
    )
